function buildGeneral2018Feature(row) {
...
async function getGeneral2018Database() {
  if (GPKG_2018_DB_PROMISE) return GPKG_2018_DB_PROMISE;

  GPKG_2018_DB_PROMISE = (async () => {
    const SQL = await ensureSqlJsReady();
    const { blob } = await fetchBlobFromZipEntry(
      `${DATA_BASE_URL}locais_votacao_2018_gkpg.zip`,
      null,
  ...
  return GPKG_2018_DB_PROMISE;
}

async function loadCensoJson2018(uf) {
  const ufNorm = String(uf || '').toUpperCase();
  if (!ufNorm) return null;
  if (CENSO_2018_CACHE.has(ufNorm)) return CENSO_2018_CACHE.get(ufNorm);

  const promise = (async () => {
    const zipUrl = `${DATA_BASE_URL}Censo 2018/censo_2018_${ufNorm}.zip`;
    const filename = `censo_2018_${ufNorm}.json`;
    const { data } = await fetchJsonFromZipEntry(zipUrl, filename);
    return data;
  })();

  CENSO_2018_CACHE.set(ufNorm, promise);
  return promise;
}

function mergeGeneralCensoJson2018(baseGeo, censusJson) {
  ...
  console.log(`[2018] Censo mesclado em ${mergedCount} locais.`);
}

async function loadGeneralStateBaseFromGpkg2018(uf) {
  const ufNorm = String(uf || '').toUpperCase();
  if (!ufNorm) throw new Error('UF 2018 invalida.');
  if (GENERAL_2018_BASE_CACHE.has(ufNorm)) {
    return GENERAL_2018_BASE_CACHE.get(ufNorm);
  }
  ...
  const promise = (async () => {
    ...
    const baseGeo = {
      ...
      features: rows.map((row) => buildGeneral2018Feature(row))
    };

    try {
      const censusJson = await loadCensoJson2018(ufNorm);
      mergeGeneralCensoJson2018(baseGeo, censusJson);
    } catch (error) {
      console.warn(`[2018] Censo nao carregado para ${ufNorm}:`, error);
    }

    return baseGeo;
  })();

  GENERAL_2018_BASE_CACHE.set(ufNorm, promise);
  return promise;
}
...
async function loadGeneralScopeBase2018(ufs, resultKeys) {
  ...
  if (!features.length) {
    throw new Error('Nenhum local do GPKG 2018 bateu com os resultados JSON.');
  }

  return { type: 'FeatureCollection', features };
}

async function loadGeneralMajoritariaJson2018(cargo, uf, turno) {
  const ufNorm = String(uf || '').toUpperCase();
  const isSenador = cargo === 'senador';
  const zipUrl = isSenador
    ? `${DATA_BASE_URL}Majoritarias 2018/senador_2018_ord_t${turno}_${ufNorm}.zip`
    : `${DATA_BASE_URL}Majoritarias 2018/${cargo}_2018_t${turno}_${ufNorm}.zip`;
  const filename = isSenador
    ? `senador_2018_ord_t${turno}_${ufNorm}.json`
    : `${cargo}_2018_t${turno}_${ufNorm}.json`;
  const { data } = await fetchJsonFromZipEntry(zipUrl, filename);
  return data;
}

function mergeGeneralJsonPayloads2018(payloads) {
  const merged = {
    METADATA: {
      cand_names: {},
...
function applyGeneralMajoritariaJsonToGeojson2018(geojson, fullJson, turnoKey) {
...
async function loadMajoritariaCargo2018(cargo, uf) {
  const ufs = (cargo === 'presidente' && String(uf).toUpperCase() === 'BR')
    ? ALL_STATE_SIGLAS
    : [String(uf || '').toUpperCase()];

  const turno1Payloads = (await Promise.all(
    ufs.map((sigla) => loadGeneralMajoritariaJson2018(cargo, sigla, 1).catch(() => null))
  )).filter((payload) => payload?.RESULTS);

  if (!turno1Payloads.length) return null;

  const mergedTurno1 = mergeGeneralJsonPayloads2018(turno1Payloads);
  const resultKeys = new Set(Object.keys(mergedTurno1.RESULTS || {}));

  let mergedTurno2 = null;
  if (cargo !== 'senador') {
    const turno2Payloads = (await Promise.all(
      ufs.map((sigla) => loadGeneralMajoritariaJson2018(cargo, sigla, 2).catch(() => null))
    )).filter((payload) => payload?.RESULTS);

    if (turno2Payloads.length) {
      mergedTurno2 = mergeGeneralJsonPayloads2018(turno2Payloads);
      Object.keys(mergedTurno2.RESULTS || {}).forEach((key) => resultKeys.add(key));
    }
  }

  const geojson = await loadGeneralScopeBase2018(ufs, resultKeys);
  applyGeneralMajoritariaJsonToGeojson2018(geojson, mergedTurno1, '1T');
  if (mergedTurno2) {
    applyGeneralMajoritariaJsonToGeojson2018(geojson, mergedTurno2, '2T');
  }
...
async function buildDeputyBaseGeojson2018(uf) {
  const resultKeys = collectLoadedDeputyResultKeys();
  const baseGeo = await loadGeneralStateBaseFromGpkg2018(uf);
  return filterGeneralFeatures2018(baseGeo, resultKeys);
}

async function onClickLoadData_Geral_2018() {
  ...
  try {
    ...
    const results = await Promise.all(cargos.map((cargo) => loadMajoritariaCargo2018(cargo, ufToLoad)));
    let dataFound = false;
    ...
    if (!dataFound) {
      throw new Error('Nenhum dado JSON encontrado para 2018.');
    }

    finalizeGeneralLoadUI(ufToLoad);
    showToast(`Dados de ${ufToLoad} (${year}) carregados!`, 'success');
  } catch (error) {
    console.error('[2018] Falha ao carregar gerais:', error);
...
async function onClickLoadData_Deputies_2018(uf, year) {
  ...
  try {
    const baseGeo = await buildDeputyBaseGeojson2018(uf);
    if (!baseGeo?.features?.length) {
      throw new Error('Nenhum local de deputado 2018 encontrado no GPKG.');
    }
    ...
    console.error('[2018] ERRO Deputados:', error);
